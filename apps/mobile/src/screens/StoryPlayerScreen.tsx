...
import type { PlaybackContext } from "../delivery/source";
  ...
  return (
    <View style={styles.timelineRegion}>
      ...
      <View
        ...
        style={[
          styles.timelineTouchTarget,
          disabled && styles.timelineDisabled,
        ]}
      >
...
export function StoryPlayerScreen({
  ...
  const player = useStoryPlayer(story, context);
  const {
    state,
    choice,
    isHydrating,
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={["top", "bottom"]}>
      {(player.transport === "preparing" ||
        player.transport === "buffering" ||
        player.transport === "failed") && (
        <View style={{ padding: 16 }}>
          <Text style={{ color: "#F3F6F1" }}>
            {player.transportError ??
              (player.transport === "buffering"
                ? "Bağlantı bekleniyor…"
                : "Ses hazırlanıyor…")}
          </Text>
          {player.transport === "failed" && (
            <Pressable onPress={player.retryTransport}>
              <Text style={{ color: "#F7B955", padding: 12 }}>Tekrar dene</Text>
            </Pressable>
          )}
        </View>
      )}
      ...
      <View style={styles.screen}>
        {onBack && (
          ...
          >
            <Text style={{ color: palette.mist, fontSize: 15 }}>
              ‹ Kitaplığa dön
            </Text>
          </Pressable>
        )}
        <View style={styles.header}>
          <View style={styles.episodeMark}>
            <Text style={styles.episodeNumber}>{story.episode.number}</Text>
          </View>
          <View style={styles.headerText}>
            <Text style={styles.seriesTitle}>{story.title}</Text>
            <Text style={styles.episodeTitle}>{story.episode.title}</Text>
        ...
        <View style={styles.controls}>
          ...
          <View style={styles.transportRow}>
            ...
            <Pressable
              ...
              onPress={() => {
                if (sectionNavigation.next)
                  navigateToSection(sectionNavigation.next);
              }}
              style={({ pressed }) => [
                styles.sectionSkipButton,
                pressed && styles.sectionSkipButtonPressed,
        ...
        <View style={styles.modalBackdrop}>
          <View style={styles.resumeCard}>
            ...
            <Text style={styles.resumeText}>
              Bu bölümü kaldığın yerden sürdürebilir veya en baştan
              dinleyebilirsin.
            </Text>
            <Pressable
              accessibilityRole="button"
              ...
              style={({ pressed }) => [
                styles.resumePrimary,
                pressed && styles.buttonPressed,
              ]}
            >
              <Text style={styles.resumePrimaryText}>
