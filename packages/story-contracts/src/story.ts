...
export const AudioSegmentSchema = z
  ...
  .superRefine((segment, context) => {
    if (
      segment.ttsText &&
      segment.ttsText.replace(/\[[^\]\r\n]+\] ?/g, "") !== segment.text
    ) {
      context.addIssue({
        code: "custom",
        path: ["ttsText"],
        message: "Inline audio tags must preserve the spoken text.",
      });
    }
  });
...
export const StorySchema = z
  ...
  .superRefine((story, context) => {
    ...
    let cursor: string | null = story.entryNodeId;
    while (cursor && story.nodes[cursor]) {
      if (visited.has(cursor)) {
        context.addIssue({
          code: "custom",
          path: ["nodes", cursor],
          message: "Story must terminate; cycles are not supported.",
        });
        break;
      }
      visited.add(cursor);
      cursor = story.nodes[cursor].nextNodeId;
    }
